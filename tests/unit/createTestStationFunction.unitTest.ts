...
describe("createTestStation function", () => {
  ...
  describe("with no payload (at all)", () => {
    ...
    it("throws Invalid Payload error", async () => {
      expect.assertions(1);
      // @ts-ignore
      await createTestStation(emptyEvent).catch((e) => {
  ...
  describe("with payload and testStationId in pathParameters", () => {
    it("invoked the processRequest function", async () => {
      ...
      expect.assertions(2);
      // @ts-ignore
      await createTestStation(event)
      expect(svc).toHaveBeenCalledTimes(1);
      expect(svc).toHaveBeenCalledWith(event.body, TARGET.TEST_STATIONS, CHANGE.CREATE, event.pathParameters.testStationId);
    });
  });
});
