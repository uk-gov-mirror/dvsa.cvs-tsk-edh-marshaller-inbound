...
describe("ConfigurationUtil", () => {
    ...
    context("when calling getConfig", () => {
        it("returns the full config object", () => {
            ...
            expect(Object.keys(conf)).toEqual(expect.arrayContaining(["functions", "serverless", "targets"]));
            expect(Object.keys(conf.targets)).toEqual(expect.arrayContaining(["test-stations"]));
        });
    });
